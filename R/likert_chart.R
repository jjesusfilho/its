#' Plota echarts para escala likert
#'
#' @param df Data.frame
#' @param grupo Variável agrupadora
#' @param variavel Variável com ítens likert
#' @param titulo Título do gráfico
#'
#' @details Essa função plota apenas cotegorias com 2, 3, 5 e 7
#'     níveis. Além disso, ela remove NAs.
#'
#' @return Gráfico com possibilidade de ser baixado
#' @export
#'
#' @examples
likert_bar <- function(df, grupo, variavel, titulo)
{
  ...
  if (categorias == 7){
    ...
    dd %>% dplyr::mutate(total = .data[[v[[1]]]] + .data[[v[[2]]]] + .data[[v[[3]]]] + .data[[v[[4]]]] + .data[[v[[5]]]]+ .data[[v[[6]]]] + .data[[v[[7]]]]) %>%
      echarts4r::mutate(total= rowSums(.[2:8])) %>%
      echarts4r::mutate_at(2:8,list(~(.*100/total) %>% round(1))) %>%
      echarts4r::mutate(total = NULL) %>%
